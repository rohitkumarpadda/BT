...
async function main() {
  ...
  if (network === "hardhat" || network === "localhost" || network === "unknown") {
    ...
    const ONE_HOUR = 3600;
    await (await gov.connect(proposer).createProposal(
  ...
  console.log(`✓ ABI + address exported → frontend/src/contract/GovernanceVoting.json`);
  console.log("\n══════════════════════════════════════════");
