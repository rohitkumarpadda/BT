...
export function Web3Provider({ children }) {
  ...
  useEffect(() => {
    ...
    const ro = new ethers.Contract(contractData.address, contractData.abi, rpcProvider);
    setRoContract(ro);
  }, []);
  ...
  async function _buildSession(address) {
    try {
      ...
      const rwContract = new ethers.Contract(contractData.address, contractData.abi, web3Signer);
      const roRead     = new ethers.Contract(contractData.address, contractData.abi, web3Provider);
  ...
  // ── Voter info refresh ──────────────────────────────────────────────────
  const _refreshVoterInfo = useCallback(async (addr, con) => {
    try {
  ...
  const onContractEvent = useCallback((eventName, handler) => {
  ...
  return (
    <Web3Ctx.Provider value={{
      provider, signer, contract, roContract,
      account, chainId, connecting,
      voterInfo, refreshVoterInfo,
