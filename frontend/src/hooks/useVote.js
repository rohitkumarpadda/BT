...
export function useVote() {
  ...
  const castVote = useCallback(async (proposalId, optionIndex) => {
    if (!contract || !account) {
...
export function useDelegate() {
  ...
  const delegate = useCallback(async (delegateAddress) => {
    if (!contract || !account) {
      toast.error("Connect your wallet first");
      return false;
    }
...
function _parseError(e) {
  ...
  if (msg.includes("AlreadyVoted"))        return "You've already voted on this proposal";
  if (msg.includes("VotingClosed"))        return "Voting period has ended";
  if (msg.includes("NotRegistered") || msg.includes("AccessControl")) return "You are not a registered voter";
  if (msg.includes("NoVotingWeight"))      return "No voting weight (check delegation)";
