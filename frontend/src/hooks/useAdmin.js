...
export function useProposalManagement() {
	...
	const editProposal = useCallback(
		async (proposalId, description, options, newDeadline) => {
			if (!contract || !account) {
				toast.error('Connect your wallet first');
				return false;
			}
			...
			setLoading(true);
			const tid = toast.loading('Editing proposal…');
			try {
				const tx = await contract.editProposal(
					proposalId,
					description,
					options,
					newDeadline,
				...
				await tx.wait();
				toast.success('Proposal edited!', { id: tid });
				return true;
			} catch (e) {
				toast.error(_parseAdminError(e), { id: tid });
				return false;
	...
	const cancelProposal = useCallback(
		async (proposalId, reason = '') => {
			...
			setLoading(true);
			const tid = toast.loading('Cancelling proposal…');
			try {
				const tx = await contract.cancelProposal(proposalId, reason);
				await tx.wait();
				toast.success('Proposal cancelled', { id: tid });
				return true;
			} catch (e) {
				toast.error(_parseAdminError(e), { id: tid });
				return false;
	...
	return { editProposal, cancelProposal, loading };
}
...
function _parseAdminError(e) {
	...
	if (msg.includes('InvalidDuration'))
